...
def teacher_agent(
    ...
    try:
        ...
        effective_difficulty = (
            ...
            else lesson_state.get("difficulty", "beginner")
        )
        next_step["difficulty"] = effective_difficulty

        segment_type = next_step.get("segment_type", "").lower()
        if segment_type in {"question", "practice", "assessment"}:
            next_step["teacher_status"] = "WAITING_FOR_STUDENT"
            next_step["message"] = "Present this question or practice activity and wait for the student's answer."
        else:
            next_step["teacher_status"] = "TEACHING"
            next_step["message"] = "Present this teaching segment to the student."
