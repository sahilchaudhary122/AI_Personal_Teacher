...
def get_or_create_lesson_state(
    lesson_id: str,
    student_id: str,
) -> Dict[str, Any]:
    """
    Get existing lesson state or initialize a new one.
    """
    state = get_lesson_state(lesson_id, student_id)
    if state:
        return state

    # Fetch lesson details to derive initial concept properly
    lesson_result = (
        supabase
        .table("lessons")
        .select("subject, topic, difficulty, learning_objectives, lesson_content")
        .eq("id", lesson_id)
        .single()
        .execute()
    )
    ...
    # Explicitly derive initial concept
    learning_objectives = lesson.get("learning_objectives") or []
    segments = lesson.get("lesson_content", {}).get("segments") or []
    
    initial_concept = ""
    if segments:
        first_segment = segments[0]
        objective_index = first_segment.get("objective_index")
        if (isinstance(objective_index, int) and 0 <= objective_index < len(learning_objectives)):
            initial_concept = learning_objectives[objective_index]
        else:
            initial_concept = first_segment.get("concept") or first_segment.get("title") or "Introduction"
